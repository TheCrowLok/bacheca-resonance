...
export const PostCard = memo(function PostCard({ post, index }: PostCardProps) {
  ...
  // LOGICA ADMIN - MODALITÀ DIO
  const urlParams = new URLSearchParams(window.location.search);
  const adminPass = urlParams.get('admin');
  const isAdmin = adminPass === "admin123"; // Assicurati che coincida con routes.ts

  const handleDelete = (e: React.MouseEvent) => {
    e.stopPropagation(); // Evita di aprire lo zoom quando clicchi il cestino
  ...
  return (
    <>
      <motion.div
        ...
        className={`sticky-note relative w-full max-w-sm mx-auto p-5 sm:p-6 ${colorClass} flex flex-col gap-4 aspect-square max-h-[400px] shadow-xl`}
        style={{ transformOrigin: "center center" }}
      >
        {/* TASTO ELIMINA SEGRETO (Solo se Admin) */}
        {isAdmin && (
          <button 
            onClick={handleDelete}
            className="absolute -top-3 -right-3 bg-red-600 text-white rounded-full p-2 shadow-2xl z-50 hover:scale-125 transition-transform border-2 border-white"
          >
      ...
      {/* MODALE PER L'IMMAGINE INGRANDITA - RIPRISTINATO STILE POLAROID */}
      <AnimatePresence>
        {isZoomed && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              {/* L'immagine con il bordo bianco stile Polaroid */}
              <img
                src={post.imageUrl!}
              ...
              {/* IL PEZZO CHE MANCAVA: Il testo sotto l'immagine */}
